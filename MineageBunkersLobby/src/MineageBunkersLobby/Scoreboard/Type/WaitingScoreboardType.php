...
class WaitingScoreboardType extends Scoreboard{
	...
	protected function initialize() : void{
		$sCfg = MineageBunkersLobby::getInstance()->getConfig()->getNested('scoreboards')['waiting'];
		...
		$this->lines[] = [3, '  '];
		$this->lines[] = [4, TextFormat::colorize($sCfg['lines']['waiting-for'])];
		...
		foreach($this->lines as $line){
			$s = str_replace(
				...
				[
					...
					$object->getMap(),
					date('i:s', PreGame::DURATION_WAITING - $object->getWaitingCooldown()),
				],
				$line[1]);
